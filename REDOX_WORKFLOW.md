...
| `config_redox.json` | 分子氧化还原参数配置 | Task 5 |
...
```bash
# MD 配置
config_opls.json              # OPLS 力场配置（已有）

# 分子结构
start_with_electrodes.pdb     # 电极+电解质体系（已有）
pdb_bank/EC.pdb              # EC 分子（已有）
pdb_bank/DMC.pdb             # DMC 分子（已有）

# 新增：红氧参数
config_redox.json            # 红氧分子参数（已生成）
```

### 2. 编辑 config_redox.json
