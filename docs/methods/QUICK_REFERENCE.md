...
| `python scripts/cli/run_voltage_sweep.py --help` | 查看所有选项 |
| `python -c 'import sys; sys.path.insert(0, \"core\"); from redox_mc import RedoxMC; help(RedoxMC)'` | API 帮助 |
...
### 添加新分子
编辑 `configs/redox/config_redox.json`：
...
```
减少步数：
python scripts/cli/run_voltage_sweep.py --equil-steps 2000 --sample-steps 3000
```

### 问题 3: 模拟与实验相差大（>200 mV）
```
调整溶剂化能：
修改 configs/redox/config_redox.json 中的 partition_coefficient
典型范围：-10 到 +5 kJ/mol
```
...
```
增加采样时间或缩小电位步长：
python scripts/cli/run_voltage_sweep.py --sample-steps 20000 --v-step 0.05
```
...
```bash
# 1. 在 configs/redox/config_redox.json 中设置分子参数
# 2. 运行完整扫描
python scripts/cli/run_voltage_sweep.py --v-start -5 --v-end -1 --v-step 0.1
...
python scripts/cli/run_voltage_sweep.py ... --lsv-peak-v -1.7
...
for pc in -5 -3 -1 0 1; do
  echo "partition_coefficient: $pc" >> configs/redox/config_redox.json
  python scripts/cli/run_voltage_sweep.py --output-dir sweep_pc_$pc
done
```
...
3. **API 文档**：`python -c "import sys; sys.path.insert(0, "core"); from redox_mc import RedoxMC; help(RedoxMC)"`
